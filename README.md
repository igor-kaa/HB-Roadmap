...
### Приоритет и фокус департаментов

`Priority` локации нормализуется как `Critical`, `High`, `Medium` или `Low`; также принимаются короткие значения `P0`–`P3`. Неизвестное непустое значение считается ошибкой входных данных.

Каждый департамент независимо работает максимум с двумя готовыми локациями в один рабочий день. Локации выбираются сначала по приоритету, затем по порядку в estimates CSV. Заблокированная dependencies локация временно освобождает focus-slot; когда её задача разблокируется, она снова вытесняет менее приоритетную локацию. Другие департаменты имеют собственные focus-slot и могут в тот же день работать с другим набором локаций.
